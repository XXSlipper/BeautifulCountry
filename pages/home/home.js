...
Page({
  ...
  data: {
    ...
    networkState:[{state:-1},{state:-1},{state:-1},{state:-1},{state:-1}],//-2 网络请求中, -1 未加载, 0 请求失败, 1请求成功
  ...
  refreshSegView: function (index) {
    ...
    //下面实行暴力刷新
    this.setData({ segSelectedIndex: index, moveViewWidth: moveViewWidth})
    this.setData({ frashMoveView: !this.data.frashMoveView })
    this.setData({ moveViewXLocation: moveViewLocation_x })
    ...
    if(index == 0){
      ...
      this.setData({ hiddenSearchBar:true, newsListSwiperH: newsListSwiperH})
      ...
      query1.select('.questionContainer').boundingClientRect(function (rect1) {
        ...
        self.setData({ hiddenSearchBar:false, newsListSwiperH: newsListSwiperH})
      ...
      this.setData({ hiddenSearchBar: false, newsListSwiperH: newsListSwiperH})
    }
  ...
  onLoad: function (options) {
    ...
    this.setData({ segContentWidth: windowWidth > totalLen ? windowWidth : totalLen })
    ...
    if(index == 0 | index == 1 | index == 2){
      ...
      networkHandle.getArticleList({
        ...
        fail: function (e) {
          ...
          self.refreshSegView(index)
          if (index == 0) {
            self.setData({ minshengList: self.data.minshengList })
          } else if (index == 1) {
            self.setData({ junshiList: self.data.junshiList })
          } else if (index == 2) {
            self.setData({ chuangyeList: self.data.chuangyeList })
          }
          wx.showToast({
            title: e.errorMsg,
      ...
      networkHandle.getQuestionList({
        ...
        success:function(e){
          ...
          self.setData({ questionList: newQuestionList })
          self.refreshSegView(index)
        },
        fail:function(e){
...
//轮播图片点击事件
  ...
  clickedOnSeg: function (e){
    ...
    this.setData({ segSelectedIndex: e.currentTarget.dataset.index })
...
//手势左右滑动分区内容页
  listSwiperChange: function (e){
    ...
    this.setData({ segSelectedIndex: e.detail.current})
